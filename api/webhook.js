...
async function saveMultipleWorkouts(phone, workouts, daysAgo = 0) {
  const date = new Date();
  date.setDate(date.getDate() - daysAgo);
  date.setHours(12, 0, 0, 0);
  ...
  try {
    ...
    try {
      ...
      if (parsed.action === "get_last") {
        ...
        if (!last) {
          await sendWhatsApp(phone, `Nenhum registro de ${normalizeExercise(parsed.exercise)} encontrado, ${user.name}.`);
        } else {
      ...
      if (parsed.action === "change_name") {
        await supabase.from("users").update({ name: parsed.name }).eq("phone", phone);
        await sendWhatsApp(phone, `✅ Pronto! Agora vou te chamar de ${parsed.name}. 😊`);
        return res.status(200).json({ ok: true });
      }
