...
// Sugestões de exercícios similares para quando não encontrar
const EXERCISE_SUGGESTIONS = {
  pulley: ["Pulley Frente", "Pulley Neutro", "Pulley Atrás"],
  puxada: ["Pulley Frente", "Barra Fixa", "Remada Baixa"],
  remada: ["Remada Curvada", "Remada Unilateral", "Remada Baixa"],
  triceps: ["Tríceps Corda", "Tríceps Pulley", "Tríceps Testa"],
  biceps: ["Rosca Direta", "Rosca Martelo", "Rosca Concentrada"],
  ombro: ["Desenvolvimento", "Elevação Lateral", "Elevação Frontal"],
  costas: ["Pulley Frente", "Remada Curvada", "Levantamento Terra"],
  peito: ["Supino Reto", "Supino Inclinado", "Crucifixo"],
  perna: ["Agachamento Livre", "Leg Press", "Cadeira Extensora"],
  gluteo: ["Hip Thrust", "Agachamento Sumô", "Cadeira Abdutora"],
};
...
async function deleteLastWorkout(phone, exercise) {
  let query = supabase
    .from("workouts").select("id, exercise, sets, reps, weight_kg")
    .eq("user_phone", phone)
    .order("created_at", { ascending: false }).limit(1);
  if (exercise) {
    query = query.ilike("exercise", `%${normalizeExercise(exercise)}%`);
...
async function updateLastWorkout(phone, exercise, newData) {
  ...
  const updateFields = {};
  if (newData.sets) updateFields.sets = newData.sets;
  if (newData.reps) updateFields.reps = newData.reps;
  if (newData.weight_kg !== undefined) updateFields.weight_kg = newData.weight_kg || null;
  if (newData.new_exercise) updateFields.exercise = normalizeExercise(newData.new_exercise);

  await supabase.from("workouts").update(updateFields).eq("id", data[0].id);
  return { ...data[0], ...updateFields };
}
...
Você APENAS entende sobre treinos, exercícios, academia e evolução física. Se perguntarem sobre outro assunto, pergunte se quer registrar um treino ou ver o histórico. Se for uma pergunta sobre musculação/exercício (ex: "Pulley é para as costas?"), responda normalmente em texto curto e educativo.
...
Múltiplos exercícios de uma vez:
...
Deletar por posição na lista (ex: "exclui o primeiro", "apaga o exercício 3"):
{"action":"delete_by_position","position":1}

Alterar/corrigir último registro (mantém carga anterior se não mencionar nova):
{"action":"update_last","exercise":"supino","sets":3,"reps":12,"weight_kg":30}

Trocar exercício por outro (ex: "troca o supino por agachamento"):
{"action":"update_last","exercise":"supino","new_exercise":"agachamento"}

Alterar por posição (ex: "altera o exercício 2 para 3x12 30kg"):
{"action":"update_by_position","position":2,"sets":3,"reps":12,"weight_kg":30}

Mudar nome — APENAS sinaliza intenção, confirmação será pedida depois:
...
Treinos em academia específica:
{"action":"get_gym_history","gym":"SmartFit"}

PR em academia específica:
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    try {
      ...
      // ── solicitar mudança de nome (com confirmação) ──
      if (parsed.action === "change_name_request") {
        pendingNameChanges[phone] = parsed.name;
        await sendWhatsApp(phone, `Quer mesmo trocar seu nome para *${parsed.name}*? Responda "sim" para confirmar ou qualquer outra coisa para cancelar.`);
        return res.status(200).json({ ok: true });
      }
