...
Se perguntarem sobre outro assunto, redirecione para treino, histórico ou hidratação.

IMPORTANTE:
- Quando a intenção exigir ação do sistema, responda SOMENTE com JSON puro.
- Não escreva explicação antes do JSON.
- Não escreva explicação depois do JSON.
- Não use blocos de código.
- Não use markdown no JSON.
- Se for apenas conversa normal ou dúvida sobre musculação, responda em texto curto e natural.
- Se o usuário perguntar o próprio nome, use:
{"action":"get_name"}

Quando registrar UM exercício:
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!parsed) {
      if (plainReply) {
        await sendWhatsApp(phone, plainReply);
      } else {
        await sendWhatsApp(phone, `Não entendi muito bem. Quer registrar treino, ver histórico ou hidratação? 💪`);
      }
      return res.status(200).json({ ok: true });
    }
    ...
    if (parsed.action === "get_name") {
      if (user.name) {
        await sendWhatsApp(phone, `Seu nome salvo aqui é *${user.name}* 💪`);
      } else {
        await sendWhatsApp(phone, `Ainda não tenho seu nome salvo. Como você quer ser chamado?`);
      }
      return res.status(200).json({ ok: true });
    }
    ...
    if (parsed.action === "change_name") {
      ...
      if (!newName) {
        await sendWhatsApp(phone, `Não consegui entender o nome novo. Me fala assim: "quero que me chame de João".`);
        return res.status(200).json({ ok: true });
      }
    ...
    if (parsed.action === "save_workout") {
      ...
      if (history.length > 0 && parsed.weight_kg) {
        const weightHistory = history.filter((w) => w.weight_kg);
        const lastWeight = history[0]?.weight_kg ?? null;
        const maxWeight = weightHistory.length
          ? Math.max(...weightHistory.map((w) => w.weight_kg))
          : null;
        const last3 = history.slice(0, 3).filter((w) => w.weight_kg);
        const diff = lastWeight !== null ? parsed.weight_kg - lastWeight : null;
    ...
    if (parsed.action === "get_last_session") {
      ...
      if (!workouts.length) {
        ...
        await sendWhatsApp(
          phone,
          `🏋️ Último treino de *${parsed.muscle_group}* (${date}), ${user.name}:\n\n${list}`,
        );
      }
    ...
    if (parsed.action === "log_water") {
      ...
      if (!amountMl) {
        await sendWhatsApp(phone, `Não consegui entender a quantidade de água. Ex: "bebi 500ml"`);
        return res.status(200).json({ ok: true });
      }
    ...
    if (parsed.action === "get_water") {
      ...
      if (totalMl === 0) {
        await sendWhatsApp(
          phone,
          `💧 Você ainda não registrou água hoje, ${user.name}.\n\nSua meta é ${goalMl}ml. Bora hidratar! 💪`,
        );
      } else {
        await sendWhatsApp(
          phone,
    ...
    if (plainReply) {
      ...
      await sendWhatsApp(phone, `Não consegui processar isso direito. Tenta me mandar de outro jeito 😊`);
    }
  } catch (err) {
